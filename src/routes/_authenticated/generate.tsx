...
function GenerateContent() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = (await create({ data: { amount: n } })) as unknown as {
        ...
        <form onSubmit={submit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full text-lg px-4 py-3 rounded-lg border border-gray-300 focus:border-[#0d4a3a] focus:ring-2 focus:ring-[#0d4a3a]/20 outline-none"
            />
          </div>
