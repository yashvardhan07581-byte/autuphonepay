  ...
  try {
    ...
    if (inboxes.length === 0) {
      return Response.json(
        { ok: false, error: "No connected email account. Connect Gmail in Settings." },
        { status: 400 },
      );
    }
    ...
    for (const inbox of inboxes) {
      try {
        const r = await scanInbox(supabase, inbox, request.url);
        scanned += r.scanned;
        matched += r.matched;
...
async function scanInbox(
  ...
  const messages: InboxMessage[] = await fetchRecentMessages({
    ...
    withinMinutes: 5, // 10 → 5 min (faster + fewer emails)
    maxResults: 5, // 15 → 5 emails max
  ...
  for (const item of metaList) {
    ...
    if (claimErr) {
      results.push({ id, skipped: "already claimed" });
      continue;
    }
    ...
    try {
      ...
      if (!senderMatches(from)) {
        results.push({ id, skipped: "sender mismatch" });
        continue;
      }
      ...
      if (amount === null) {
        results.push({ id, skipped: "no amount" });
        continue;
      }
      ...
      const { data: alreadyUsed } = await supabase
        ...
        .maybeSingle();
      if (alreadyUsed) {
        results.push({ id, skipped: "already attributed" });
        continue;
      }
      ...
      if (candErr) {
        results.push({ id, error: candErr.message });
        continue;
      }
      ...
      if (updErr) {
        results.push({ id, skipped: "unique conflict" });
        continue;
      }
      if (!updated) {
        results.push({ id, skipped: "order no longer pending" });
        continue;
      }
      ...
      // Payment detect hote hi webhook turant bhejo (cron wait mat karo)
      try {
        const dispatchUrl = new URL("/api/public/v1/internal/dispatch-webhooks", requestUrl);
        await fetch(dispatchUrl.toString(), { method: "POST" });
      } catch {
        // Ignore — cron will pick it up within 1 minute
      }
    } catch (e: any) {
